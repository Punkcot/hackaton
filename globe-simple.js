/**
 * @file: globe-simple.js
 * @description: Упрощенная 3D модель земного шара для GitHub Pages
...
class Globe3D {
    ...
    highlightCity(cityName) {
        ...
        if (city) {
            ...
            // Подсвечиваем связанные маршруты
            this.routes.forEach(route => {
    ...
    filterByDays(selectedDays) {
        this.routes.forEach(route => {
            ...
            route.visible = hasSelectedDay;
            route.material.opacity = hasSelectedDay ? 0.6 : 0.1;
        });
    }
